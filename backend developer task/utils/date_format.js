...
const modifyDateFormat = function (value, format, timezone) {
    if (!isEmpty(value)) {
        if (isEmpty(format)) {
            format = FMT;
        }
...
function addDaysToCurrentDate () {
    let currentDate = modifyDateFormat(moment().add(1,'days'), "YYYY-MM-DD", this.defaultTimezone)
    return currentDate + " 00:00:00"
    // currentDate.setDate(currentDate.getDate() + d)
    // return modifyDateFormat(currentDate.setHours(0, 0, 0, 0), "YYYY-MM-DD HH:mm:ss");
...
const getCurrentTimeDiffBetweenUserTimezonesAndUTCInSec = function(userTimeZone){
...
module.exports = {
    ...
    getCurrentTimeDiffBetweenUserTimezonesAndUTCInSec
};
